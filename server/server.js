...
export default function App() {
  ...
  // Fetch unique attribute list for the current active collection
  const loadFilterOptions = useCallback((col) => {
    fetch(`${API_BASE_URL}/api/filter-options/${col}`)
      .then(r => r.json())
      .then(opts => {
        if (Array.isArray(opts)) setFilterOptions(opts);
      })
  ...
  const loadData = useCallback(() => {
    setLoading(true);
    fetch(`${API_BASE_URL}/api/data/${collection}?page=${page}&limit=10&search=${encodeURIComponent(searchTerm)}&filterAttr=${encodeURIComponent(filterAttr)}`)
      .then(r => r.json())
      .then(res => {
        setItems(res.data || []);
  ...
  }, [collection, page, searchTerm, filterAttr]);

  // When switching collections, reset search & filter attribute
  ...
  const getFilterLabel = () => {
    if (collection === 'customers') return 'State:';
    if (collection === 'products') return 'Category:';
    if (collection === 'orders') return 'Status:';
    return 'Filter:';
  };
    ...
    <div style={{ backgroundColor: '#09090b', minHeight: '100vh', padding: '24px', color: '#fafafa', fontFamily: 'Inter, system-ui, sans-serif' }}>
      ...
      {activePage === 'crud_data' && (
        <>
          ...
          {/* Search Bar + Collection-Specific Attribute Filter Dropdown */}
          <div style={{ background: '#121215', padding: '14px', borderRadius: '10px', marginBottom: '16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '12px', border: '1px solid rgba(255,255,255,0.06)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '14px', flexWrap: 'wrap' }}>
              <input
                ...
                style={{ padding: '8px 14px', background: '#18181b', border: '1px solid #27272a', color: '#fff', borderRadius: '8px', width: '320px' }}
              />

              {/* Dedicated Collection Filter */}
              <label style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '13px', color: '#a1a1aa' }}>
                <span style={{ color: PALETTE.emerald, fontWeight: 'bold' }}>Filter by {getFilterLabel()}</span>
                <select
                  value={filterAttr}
                  onChange={(e) => { setFilterAttr(e.target.value); setPage(1); }}
                  style={{ padding: '7px 12px', background: '#18181b', border: '1px solid #27272a', color: '#fff', borderRadius: '6px', maxWidth: '240px' }}
                >
            ...
            <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
              <span style={{ fontSize: '13px', color: '#71717a' }}>
                {loading ? 'Querying local MongoDB...' : `Total: ${totalCount.toLocaleString('en-IN')} records | Page ${page} of ${totalPages}`}
              </span>
